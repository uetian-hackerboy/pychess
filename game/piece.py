...
class Piece:
    ...
    def get_legal_moves(self, current_coord):
        ...
        if self.name == PieceType.PAWN:
            ...
            if self.number_of_moves == 0:
                ...
                legal_coords.append((col, increment))
            legal_coords = [(c, r) for c, r in legal_coords if not self.board.representation.get((c, r), None)]
            diagonal_offsets = [1, -1]
            for offset in diagonal_offsets:
                diagonal_coord = (col + offset , increment)
                piece_at_diagonal_coord: Piece = self.board.representation.get(diagonal_coord, None)
                ...
                piece_at_side_coord : Piece = self.board.representation.get(side_coord, None)
            ...
            for offset in offsets:
                ...
                piece_at_coord = self.board.representation.get(new_coord, None)
            ...
            for col_offset, row_offset in directions:
                self.add_legal_moves_in_direction(col, row, col_offset, row_offset, legal_coords)
            ...
            if self.number_of_moves == 0:
                ...
                for i in range(1, 5):
                    ...
                    right_piece = self.board.representation.get(right_coord, None)
                    if right_piece is not None:
                        right_pieces.append(right_piece)
                    left_piece = self.board.representation.get(left_coord, None)
    ...
    def add_legal_moves_in_direction(self, col, row, col_offset, row_offset, legal_coords):
        new_coord = (col + col_offset, row + row_offset)
        if 0 <= new_coord[0] <= 7 and 0 <= new_coord[1] <= 7:
            piece_at_coord = self.board.representation.get(new_coord, None)
            if piece_at_coord is None:
                legal_coords.append(new_coord)
                self.add_legal_moves_in_direction(new_coord[0], new_coord[1], col_offset, row_offset, legal_coords)
